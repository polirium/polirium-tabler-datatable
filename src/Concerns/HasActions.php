...
use Polirium\Datatable\Providers\SupportLivewireVersions;
...
trait HasActions
{
    ...
    public function prepareActionsResources(): ?string
    {
        ...
        if (intval(config('polirium-datatable.cache_ttl')) > 0) {
            $records = strval(Cache::remember('pg-resource-icons-json', intval(config('polirium-datatable.cache_ttl')), $closure));
        } else {
    ...
    {
        $paths = (array) config('polirium-datatable.icon_resources.paths');
        $allowed = (array) config('polirium-datatable.icon_resources.allowed');
        $iconAttributes = (array) config('polirium-datatable.icon_resources.attributes');
    ...
    public function prepareActionRulesForRows(mixed $row, ?object $loop = null): array
    {
        ...
        if (intval(config('polirium-datatable.cache_ttl') > 0)) {
            ...
            $formattedRules = Cache::remember($cacheKey, intval(config('polirium-datatable.cache_ttl')), function () use ($closure, $row, $loop) {
