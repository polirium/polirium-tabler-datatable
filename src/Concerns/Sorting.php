...
trait Sorting
{
    ...
    {
        if ($this->multiSort && array_key_exists($field, $this->sortArray)) {
            return ($this->sortArray[$field] == 'desc') ? 'polirium-datatable::icons.chevron-up' : 'polirium-datatable::icons.chevron-down';
        } elseif ($this->multiSort) {
            return 'polirium-datatable::icons.chevron-up-down';
        } else {
            if ($this->sortField !== $field) {
                return 'polirium-datatable::icons.chevron-up-down';
            } elseif ($this->sortDirection == 'desc') {
                return 'polirium-datatable::icons.chevron-up';
            } else {
                return 'polirium-datatable::icons.chevron-down';
            }
        }
    }
