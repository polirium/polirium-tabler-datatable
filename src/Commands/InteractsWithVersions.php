...
class InteractsWithVersions
{
    ...
    public function searchPackage(InstalledRepositoryInterface $localRepo): array
    {
        foreach ($localRepo->getPackages() as $package) {
            if ($package->getName() === 'power-components/polirium-datatable') {
                return [
    ...
    public function getLatestVersion(): string
    {
        ...
        $resolver = static::$latestVersionResolver ?? function () {
            $json = file_get_contents(
                'https://packagist.org/p2/power-components/polirium-datatable.json'
            );
            ...
            $version = collect($package['packages']['power-components/polirium-datatable'])
                ->first()['version'];
