...
@php
    $framework = config('polirium-datatable.plugins.select');
...
$params = [
    ...
    'framework' => $framework[config('polirium-datatable.plugins.select.default')],
];
        ...
        <div class="{{ theme_style($theme, 'filterSelect.base') }}">
            ...
            >
                @if (!data_get($params, 'options.disableOptionAll', false))
                    <option value="">{{ trans('polirium-datatable::datatable.multi_select.all') }}</option>
                @endif
